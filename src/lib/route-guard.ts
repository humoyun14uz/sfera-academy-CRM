...
export const ROLES = ['Super Admin', 'Manager', 'Teacher', 'Finance', 'Student'] as const
...
const ROLE_PERMISSIONS: Record<Role, readonly Permission[]> = {
  ...
  Student: ['dashboard.read', 'students.read', 'courses.read', 'tasks.read', 'settings.read'],
}

export function normalizeRole(role?: string): Role {
  const normalized = role?.trim().toLowerCase().replace(/[_-]/g, ' ')
  if (normalized === 'superadmin' || normalized === 'super admin' || normalized === 'admin' || normalized === 'administrator') return 'Super Admin'
...
export function isKnownRole(role?: string): boolean {
  const normalized = role?.trim().toLowerCase().replace(/[_-]/g, ' ')
  return ['superadmin', 'super admin', 'admin', 'administrator', 'manager', 'teacher', 'finance', 'financial', 'student'].includes(normalized ?? '')
}
