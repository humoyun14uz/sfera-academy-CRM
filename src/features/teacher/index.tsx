...
export function TeacherDashboard({
  ...
  const shell = (content: React.ReactNode) => (
    <>
      ...
      <Main className='teacher-premium'>{content}</Main>
    </>
  )
...
function PageHeading({
  ...
  return (
    <div className='teacher-heading mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end'>
      <div>
        <p className='teacher-eyebrow'>SFERA IT Academy CRM</p>
...
function DashboardContent({
  ...
  const teacherId = useAuthStore((state) => state.auth.user?.accountNo ?? '')
  const teacherApi = useQuery({
    queryKey: ['backend', 'teacher', 'dashboard', teacherId],
    queryFn: () =>
      getApiData<TeacherDashboardApi>('/teacher/dashboard', { teacherId }),
  ...
  const english = labels.locale === 'en'
  const quickActions = [
    {
      label: english ? 'New assignment' : 'Yangi vazifa',
      icon: FileText,
      href: '/teacher/assignments',
    },
    {
      label: english ? 'Plan a lesson' : 'Dars rejalash',
      icon: CalendarPlus,
      href: '/teacher/schedule',
    },
    {
      label: english ? 'Start live class' : 'Jonli darsni boshlash',
      icon: Video,
      href: '/teacher/today',
    },
    {
      label: english ? 'Message students' : 'O‘quvchilarga yozish',
      icon: MessageSquare,
  ...
  const progressRows = [
    {
      label: english ? 'Frontend Group 2' : 'Frontend 2-guruh',
      value: 88,
      color: 'linear-gradient(90deg, #29a956, #67bd80)',
    },
    {
      label: english ? 'Python beginners' : 'Python boshlang‘ich',
      value: 76,
      color: 'linear-gradient(90deg, #218c48, #67bd80)',
    },
    {
      label: english ? 'React practice' : 'React amaliyot',
      value: 93,
      color: 'linear-gradient(90deg, #176f38, #9fd8b0)',
    },
  ]
  return (
    <>
      ...
      <BackendStatusCard
        title={english ? 'Teacher API data' : 'Teacher API ma’lumotlari'}
        description={english ? 'Workspace metrics returned by the NestJS backend.' : 'NestJS backend qaytargan o‘qituvchi ko‘rsatkichlari.'}
        status={!isBackendEnabled() ? 'disabled' : teacherApi.isPending ? 'loading' : teacherApi.isError ? 'error' : 'connected'}
        statusLabels={{
          disabled: english ? 'Demo mode' : 'Demo rejim',
          loading: english ? 'Loading' : 'Yuklanmoqda',
          error: english ? 'API error' : 'API xatosi',
          connected: english ? 'Connected' : 'Ulandi',
        }}
        errorMessage={apiErrorMessage(teacherApi.error)}
        metrics={teacherApi.data ? [
          { label: english ? 'Groups' : 'Guruhlar', value: teacherApi.data.totalGroups },
          { label: english ? 'Students' : 'O‘quvchilar', value: teacherApi.data.totalStudents },
          { label: english ? 'Average grade' : 'O‘rtacha baho', value: teacherApi.data.averageGrade },
          { label: english ? 'Attendance' : 'Davomat', value: `${teacherApi.data.attendanceRate}%` },
        ] : []}
      />
      ...
      <div
        ...
        aria-label={english ? 'Quick actions' : 'Tezkor amallar'}
      >
        {quickActions.map(({ label, icon: Icon, href }) => (
          <a key={label} href={href} className='teacher-quick-action'>
            <span className='teacher-quick-action-icon'>
      ...
      <div className='mt-6 grid gap-6 lg:grid-cols-[1.25fr_0.75fr]'>
        <Card>
          <CardHeader>
            <div className='flex items-start justify-between gap-3'>
              <div>
                <CardTitle>
                  {english ? 'Group performance' : 'Guruhlar natijasi'}
                </CardTitle>
                <CardDescription>
                  {english
                    ? 'Progress across your active groups'
                    : 'Faol guruhlaringiz bo‘yicha rivojlanish'}
                </CardDescription>
              </div>
            ...
            {progressRows.map((row) => (
              <div key={row.label}>
                ...
                <div className='teacher-progress-track'>
                  <div
                    className='teacher-progress-value'
                    style={{ width: `${row.value}%`, background: row.color }}
                  />
                </div>
                <div className='mt-1 flex justify-between text-[11px] text-muted-foreground'>
                  <span>
                    {english ? 'Attendance' : 'Davomat'} {row.value - 7}%
                  </span>
                  <span>
                    {english ? 'Average score' : 'O‘rtacha baho'}{' '}
                    {row.value - 2}%
                  </span>
                </div>
              </div>
        ...
        <Card className='teacher-focus'>
          <CardHeader>
            <div className='flex items-start justify-between gap-3'>
              <div>
                <CardTitle>
                  {english ? 'Today’s focus' : 'Bugungi fokus'}
                </CardTitle>
                <CardDescription>
                  {english
                    ? 'One clear priority for your workday'
                    : 'Bugungi ish kuningiz uchun asosiy maqsad'}
                </CardDescription>
              </div>
          ...
          <CardContent>
            <p className='text-2xl font-bold tracking-tight'>
              {english
                ...
                : '12 ta topshiriqni tekshirish'}
            </p>
            <p className='mt-2 text-sm text-sky-100/80'>
              {english
                ? 'Estimated time: 45 minutes'
                : 'Taxminiy vaqt: 45 daqiqa'}
            </p>
            <div className='mt-6 flex items-center justify-between border-t border-white/15 pt-4 text-sm'>
              <span className='flex items-center gap-2'>
                <Zap className='size-4' /> {english ? 'Priority' : 'Muhim'}
              </span>
              <a
                href='/teacher/grades'
                className='font-semibold underline underline-offset-4'
              >
                {english ? 'Open gradebook' : 'Baholarni ochish'}
              </a>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className='space-y-3'>
            {groups.map((group) => (
              <div
                key={group.id}
                className='teacher-list-item flex flex-col gap-3 rounded-xl border bg-muted/20 p-4 sm:flex-row sm:items-center sm:justify-between'
              >
                <div>
            ...
            {students.map((student) => {
              ...
              return (
                <button
                  ...
                  className='teacher-list-item flex w-full items-center justify-between rounded-lg border p-3 text-start hover:bg-muted/50'
                >
        ...
        <CardContent className='space-y-3'>
          {tasks.map((task, index) => (
            <button
              ...
              className='teacher-list-item flex w-full items-center gap-3 rounded-xl border p-3 text-start hover:bg-muted/50'
            >
              <CheckCircle2
      ...
      <div className='mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]'>
        <Card>
          <CardHeader>
            <CardTitle>
              {english ? 'Recent activity' : 'So‘nggi faoliyat'}
            </CardTitle>
            <CardDescription>
              {english
                ? 'Your latest workspace updates'
                : 'Ish maydoningizdagi so‘nggi yangilanishlar'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {[
              english
                ? 'Attendance saved for Frontend Group 2'
                : 'Frontend 2-guruh davomatini saqladingiz',
              english
                ? 'React practice assignment was published'
                : 'React amaliyot uchun vazifa chop etildi',
              english
                ? '3 students received new feedback'
                : '3 nafar o‘quvchiga yangi izoh yuborildi',
            ].map((activity, index) => (
              <div key={activity} className='teacher-activity-row'>
                <span className='teacher-activity-marker' />
                <span className='text-sm'>{activity}</span>
                <span className='text-xs text-muted-foreground'>
                  {index === 0 ? (english ? 'Now' : 'Hozir') : `${index + 1}h`}
                </span>
              </div>
            ))}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{english ? 'Weekly rhythm' : 'Haftalik ritm'}</CardTitle>
            <CardDescription>
              {english
                ? 'Small habits keep groups moving'
                : 'Kichik odatlar guruhlarni oldinga olib boradi'}
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-3'>
            {[
              {
                label: english ? 'Lessons delivered' : 'O‘tilgan darslar',
                value: '12 / 14',
              },
              {
                label: english ? 'Feedback sent' : 'Yuborilgan izohlar',
                value: '28 / 30',
              },
              {
                label: english ? 'Attendance completed' : 'Tugallangan davomat',
                value: '93%',
              },
            ].map((item) => (
              <div
    ...
    <>
      ...
      <div className='mb-4 grid gap-4 md:grid-cols-3'>
        ...
        <TeacherStat
          ...
          value='93%'
          detail={labels.locale === 'en' ? 'This month' : 'Shu oy'}
        />
      </div>
      ...
      <div className='mb-4 grid gap-4 xl:grid-cols-[1.4fr_1fr]'>
        <Card className='overflow-hidden'>
          <CardHeader>
            ...
            <CardDescription>
              {labels.locale === 'en'
                ? 'Progress and attendance at a glance'
                : 'O‘zlashtirish va davomatni bir qarashda ko‘ring'}
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-5'>
            {groups.map((group, index) => (
              <div key={group.id}>
                ...
                <div className='mt-1.5 flex justify-between text-[11px] text-muted-foreground'>
                  <span>
                    {labels.avg}: {progress[index]}%
                  </span>
                  <span>
                    {labels.attendance}: {attendance[index]}%
                  </span>
                </div>
              </div>
            ))}
          ...
          <CardHeader>
            <CardTitle>
              {labels.locale === 'en'
                ...
                : 'O‘qituvchilik ko‘rsatkichi'}
            </CardTitle>
            <CardDescription>
              {labels.locale === 'en'
                ? 'A quick view of this week'
                : 'Shu haftaning qisqa ko‘rinishi'}
            </CardDescription>
          </CardHeader>
            ...
            <div className='rounded-2xl border bg-background/70 p-4'>
              <p className='text-xs text-muted-foreground'>
                {labels.locale === 'en'
                  ...
                  : 'Eng yaxshi natijali guruh'}
              </p>
              <p className='mt-1 text-lg font-bold'>
                {groupText(groups[2], labels).name}
              </p>
              <p className='mt-1 text-sm text-emerald-600'>
                {progress[2]}% {labels.avg.toLowerCase()}
              </p>
            </div>
            <div className='grid grid-cols-2 gap-3'>
              <div className='rounded-2xl border bg-background/70 p-3'>
                <p className='text-xs text-muted-foreground'>
                  {labels.locale === 'en' ? 'Lessons' : 'Darslar'}
                </p>
                <p className='mt-1 text-2xl font-bold'>12</p>
              </div>
              <div className='rounded-2xl border bg-background/70 p-3'>
                <p className='text-xs text-muted-foreground'>
                  {labels.locale === 'en' ? 'Assignments' : 'Vazifalar'}
                </p>
      ...
      <Card>
        <CardHeader>
          <div className='flex flex-col justify-between gap-3 sm:flex-row sm:items-center'>
            <div>
              <CardTitle>{labels.groups}</CardTitle>
              <CardDescription>
                {filtered.length} {labels.groups.toLowerCase()}
              </CardDescription>
            </div>
        ...
        <CardContent className='grid gap-4 md:grid-cols-2 xl:grid-cols-3'>
          {filtered.map((group) => (
            <Card
              key={group.id}
              className='bg-muted/20 transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg'
            >
              <CardHeader>
              ...
              <CardContent className='space-y-3 text-sm'>
                ...
                <div className='flex items-center justify-between text-xs text-muted-foreground'>
                  ...
                  <span>{groupText(group, labels).room}</span>
                </div>
                <Button
                  variant='outline'
                  ...
                  asChild
                >
                  <a href={`/teacher/students?group=${group.id}`}>
                    {labels.students}
                  </a>
                </Button>
              </CardContent>
...
function SchedulePage({
  ...
  const data = todayOnly
    ? lessons.filter((lesson) => lesson.dayEn === 'Today')
    : lessons
  const days =
    labels.locale === 'en'
    ...
    <>
      <PageHeading
        ...
        description={
          todayOnly
            ? labels.locale === 'en'
              ? 'Today’s teaching plan and classroom focus.'
              : 'Bugungi darslar va o‘qituvchi ish rejasi.'
            : labels.locale === 'en'
              ? 'A visual weekly teaching board with group, room and attendance signals.'
              : 'Guruh, xona va davomat ko‘rsatkichlari bilan haftalik vizual dars jadvali.'
        }
        action={
          !todayOnly ? (
            <Button variant='outline' className='cursor-pointer'>
              <CalendarRange className='me-2 size-4' />
              {labels.locale === 'en' ? 'Current week' : 'Joriy hafta'}
            </Button>
          ) : undefined
        }
      />
      <div className='mb-5 grid gap-4 sm:grid-cols-3'>
        ...
        <TeacherStat
          icon={Trophy}
          title={labels.locale === 'en' ? 'Next focus' : 'Keyingi yo‘nalish'}
          value={
            nextLesson
              ? groupText(nextLesson, labels).name
              : labels.locale === 'en'
                ? 'No lesson'
                : 'Dars yo‘q'
          }
            ...
            <div>
              <CardTitle>
                {todayOnly
                  ...
                  : labels.locale === 'en'
                    ? 'Teaching week'
                    : 'O‘qituvchilik haftasi'}
              </CardTitle>
              <CardDescription>
                {labels.locale === 'en'
                  ? 'A compact planner instead of a plain schedule list.'
                  : 'Oddiy ro‘yxat o‘rniga ixcham va professional rejalashtirish ko‘rinishi.'}
              </CardDescription>
            </div>
            <span className='rounded-full border bg-background/75 px-3 py-1 text-xs font-semibold'>
              {data.length} {labels.locale === 'en' ? 'lessons' : 'dars'}
            </span>
          </div>
        </CardHeader>
        <CardContent className='p-4'>
          <div className='grid gap-3 md:grid-cols-2 xl:grid-cols-3'>
            {weekCards.map((slot, index) => (
              <div
                key={slot.day}
                className={`rounded-2xl border p-4 transition-all ${slot.lesson ? 'bg-card shadow-sm hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md' : 'bg-muted/20'}`}
              >
                <div className='flex items-center justify-between'>
                  ...
                  <span
                    className={`size-2 rounded-full ${slot.lesson ? 'bg-emerald-500' : 'bg-muted-foreground/30'}`}
                  />
                </div>
                {slot.lesson ? (
                  <>
                    <p className='mt-3 text-base font-bold'>
                      {groupText(slot.lesson, labels).name}
                    </p>
                    <p className='mt-1 text-xs text-muted-foreground'>
                      {groupText(slot.lesson, labels).course}
                    </p>
                    <div className='mt-4 grid grid-cols-2 gap-2 text-xs'>
                      <div className='rounded-xl border bg-muted/25 p-2.5'>
                        <p className='text-muted-foreground'>
                          {labels.locale === 'en' ? 'Time' : 'Vaqt'}
                        </p>
                      ...
                      <div className='rounded-xl border bg-muted/25 p-2.5'>
                        <p className='text-muted-foreground'>
                          {labels.locale === 'en' ? 'Room' : 'Xona'}
                        </p>
                        <p className='mt-1 font-semibold'>
                          {groupText(slot.lesson, labels).room}
                        </p>
                      </div>
                    </div>
                    <div className='mt-3 flex items-center justify-between text-xs text-muted-foreground'>
                      <span>
                        {slot.lesson.students} {labels.students.toLowerCase()}
                      </span>
                      <span>
                        {index === 0 && completed
                          ? labels.locale === 'en'
                  ...
                  <div className='mt-5 rounded-xl border border-dashed p-4 text-center'>
                    <p className='text-sm font-medium'>
                      {labels.locale === 'en' ? 'Open slot' : 'Bo‘sh vaqt'}
                    </p>
                    <p className='mt-1 text-xs text-muted-foreground'>
                      {labels.locale === 'en'
                        ? 'No lesson assigned'
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {labels.locale === 'en' ? 'Schedule health' : 'Jadval holati'}
          </CardTitle>
          <CardDescription>
            {labels.locale === 'en'
              ? 'Quick planning signals for your teaching week.'
              : 'Haftalik dars rejangiz uchun tezkor ko‘rsatkichlar.'}
          </CardDescription>
        </CardHeader>
        <CardContent className='grid gap-3 sm:grid-cols-3'>
          <div className='rounded-2xl border bg-emerald-500/[0.05] p-4'>
            <p className='text-xs text-muted-foreground'>
              {labels.locale === 'en' ? 'Attendance-ready' : 'Davomatga tayyor'}
            </p>
            <p className='mt-1 text-2xl font-bold'>100%</p>
            <p className='mt-1 text-xs text-emerald-600'>
              {labels.locale === 'en'
                ? 'All groups have students mapped'
                : 'Barcha guruhlarga o‘quvchilar biriktirilgan'}
            </p>
          </div>
          <div className='rounded-2xl border bg-primary/[0.05] p-4'>
            <p className='text-xs text-muted-foreground'>
              {labels.locale === 'en' ? 'Peak session' : 'Eng band dars'}
            </p>
            <p className='mt-1 text-2xl font-bold'>18</p>
            <p className='mt-1 text-xs text-primary'>
              {labels.locale === 'en'
                ? 'learners in one group'
                : 'bitta guruhdagi o‘quvchi'}
            </p>
          </div>
          <div className='rounded-2xl border bg-violet-500/[0.05] p-4'>
            <p className='text-xs text-muted-foreground'>
              {labels.locale === 'en' ? 'Coverage' : 'Qamrov'}
            </p>
            <p className='mt-1 text-2xl font-bold'>6/6</p>
            <p className='mt-1 text-xs text-violet-600'>
              {labels.locale === 'en'
                ? 'planner slots visible'
                : 'jadval slotlari ko‘rinmoqda'}
            </p>
          </div>
        </CardContent>
      </Card>
...
function StudentsPage({ labels }: { labels: Labels }) {
  ...
  const visibleStudents = activeGroup
    ? groupStudents(activeGroup.id)
  ...
  if (activeGroup) {
    return (
      <>
        <PageHeading
          title={groupText(activeGroup, labels).name}
          description={labels.subtitle}
          action={
            <Button
              variant='outline'
            ...
            >
              ← {labels.locale === 'en' ? 'All students' : 'Barcha o‘quvchilar'}
            </Button>
          }
        />
        <div className='mb-4 grid gap-4 md:grid-cols-3'>
          <TeacherStat
            icon={UsersRound}
            title={labels.students}
            value={String(activeGroup.students)}
          />
          <TeacherStat
            icon={Star}
            title={labels.avg}
            value='89%'
            detail={
              labels.locale === 'en' ? 'Group average' : 'Guruh o‘rtachasi'
            }
          />
          <TeacherStat
            icon={CheckCircle2}
            title={labels.attendance}
            value='94%'
            detail={labels.locale === 'en' ? 'Current month' : 'Joriy oy'}
          />
        </div>
        <Card>
          <CardHeader>
              ...
              >
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    ...
                    <p className='text-xs text-muted-foreground'>
                      {groupText(activeGroup, labels).course}
                    </p>
                  </div>
                </div>
                <Badge variant='secondary'>
                  {index % 2 ? '82%' : '91%'} {labels.avg.toLowerCase()}
                </Badge>
              </div>
            ))}
          </CardContent>
  ...
  return (
    <>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className='mt-3'>
            {labels.locale === 'en'
              ? 'Choose a group to view its students and teacher.'
              : 'Guruhni tanlab, o‘quvchilar va o‘qituvchini ko‘ring.'}
          </CardDescription>
        </CardHeader>
        <CardContent className='grid gap-3 md:grid-cols-3'>
          {groups.map((group) => (
            <button
              key={group.id}
              type='button'
              className='cursor-pointer rounded-2xl border p-4 text-left transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg'
      ...
      {!selectedGroup && visibleStudents.length > 0 && (
        <Card className='mt-4'>
          <CardHeader>
            <CardTitle>
              {labels.locale === 'en' ? 'All students' : 'Barcha o‘quvchilar'}
          ...
          <CardContent className='divide-y'>
            {visibleStudents.map((student) => (
              <div key={student} className='py-3 text-sm font-medium'>
                {student}
              </div>
            ))}
          </CardContent>
...
function AssignmentsPage({ labels }: { labels: Labels }) {
  ...
  const groupCompletion = groups.map((group, index) => ({
    group,
    value: completion[index],
  }))
  const bestGroup = groupCompletion.reduce(
    (best, item) => (item.value > best.value ? item : best),
    groupCompletion[0]
  )

  useEffect(() => {
    const shared: SharedAssignment[] = items.map((item, index) => ({
      id: `teacher-task-${index}`,
      title: item.title,
  ...
  const add = () => {
    ...
    if (!name || name.length < 3) {
      toast.error(labels.requiredTask)
      return
    }
    if (editingIndex === null) {
      setItems((current) => [...current, { title: name, groupId: taskGroup }])
      toast.success(labels.created)
    } else {
      setItems((current) =>
        current.map((item, index) =>
          index === editingIndex
            ? { ...item, title: name, groupId: taskGroup }
            : item
        )
      )
  ...
  const edit = (index: number) => {
    ...
    setTaskGroup(items[index].groupId)
    setCreateOpen(true)
  }
    ...
    <>
      ...
      <div className='mb-4 grid gap-4 md:grid-cols-3'>
        ...
        <TeacherStat
          ...
          value='78%'
          detail={
            labels.locale === 'en' ? 'Overall completion' : 'Umumiy bajarilish'
          }
        />
        <TeacherStat
          icon={Star}
          title={labels.locale === 'en' ? 'Top group' : 'Top guruh'}
          value={groupText(bestGroup.group, labels).name}
          detail={`${bestGroup.value}% ${labels.completion.toLowerCase()}`}
        />
      </div>
      ...
      <div className='mb-4 grid gap-4 xl:grid-cols-[1.1fr_0.9fr]'>
        <Card>
          <CardHeader>
            <CardTitle>
              {labels.locale === 'en'
                ? 'Completion by group'
                : 'Guruhlar bo‘yicha bajarilish'}
            </CardTitle>
            <CardDescription>
              {labels.locale === 'en'
                ? 'Submission rate across your groups'
                : 'Guruhlar kesimida topshirish darajasi'}
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-5'>
            {groupCompletion.map(({ group, value: rate }) => (
              <div key={group.id}>
                ...
                <div className='mt-1.5 flex justify-between text-[11px] text-muted-foreground'>
                  ...
                  <span>
                    {labels.locale === 'en'
                      ? `${Math.round((group.students * rate) / 100)} submitted`
                      : `${Math.round((group.students * rate) / 100)} topshirgan`}
                  </span>
                </div>
              </div>
            ))}
        ...
        <Card className='bg-gradient-to-br from-primary/[0.07] via-card to-emerald-500/[0.05]'>
          <CardHeader>
            <CardTitle>
              {labels.locale === 'en' ? 'Assignment pulse' : 'Vazifalar ritmi'}
            </CardTitle>
            <CardDescription>
              {labels.locale === 'en'
                ? 'What needs your attention next'
                : 'Keyingi e’tibor talab qiladigan ko‘rsatkichlar'}
            </CardDescription>
          </CardHeader>
          <CardContent className='grid gap-3'>
            <div className='rounded-2xl border bg-background/70 p-4'>
              <div className='flex items-center gap-2 text-xs text-muted-foreground'>
                ...
                {labels.locale === 'en' ? 'On time' : 'O‘z vaqtida'}
              </div>
              <p className='mt-2 text-3xl font-bold'>72%</p>
              <p className='mt-1 text-xs text-emerald-600'>
                {labels.locale === 'en'
                  ? 'Healthy delivery signal'
                  : 'Topshirish ko‘rsatkichi yaxshi'}
              </p>
            </div>
            <div className='grid grid-cols-2 gap-3'>
              <div className='rounded-2xl border bg-background/70 p-4'>
                <p className='text-xs text-muted-foreground'>
                  {labels.locale === 'en' ? 'Needs review' : 'Tekshiruvda'}
                </p>
                <p className='mt-1 text-2xl font-bold text-amber-600'>9</p>
              </div>
              <div className='rounded-2xl border bg-background/70 p-4'>
                <p className='text-xs text-muted-foreground'>
                  {labels.locale === 'en' ? 'Best group' : 'Eng yaxshi guruh'}
                </p>
                <p className='mt-1 truncate text-sm font-bold'>
                  {groupText(bestGroup.group, labels).name}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {labels.locale === 'en'
              ? 'Every assignment is tied to a group.'
              : 'Har bir vazifa aniq guruhga biriktiriladi.'}
          </CardDescription>
        </CardHeader>
        <CardContent className='space-y-2'>
          {items.map((item, index) => {
            const group =
              groups.find((entry) => entry.id === item.groupId) ?? groups[0]
            return (
              <div
                key={`${item.title}-${index}`}
                className='flex flex-col gap-3 rounded-2xl border p-4 transition-all hover:border-primary/35 hover:bg-muted/20 sm:flex-row sm:items-center'
              >
                ...
                <div className='min-w-0 flex-1'>
                  ...
                  <p className='mt-1 text-xs text-muted-foreground'>
                    {groupText(group, labels).name} · {group.students}{' '}
                    {labels.students.toLowerCase()}
                  </p>
                </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className='sm:max-w-[560px]'>
          <DialogHeader>
            <DialogTitle>
              {editingIndex === null ? labels.create : labels.editTask}
            </DialogTitle>
            <DialogDescription>
              {labels.locale === 'en'
                ? 'Select the group first, then assign the task to its learners.'
                : 'Avval guruhni tanlang, keyin shu guruh o‘quvchilariga vazifa bering.'}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button className='cursor-pointer' onClick={add}>
              {editingIndex === null ? labels.create : labels.save}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function GradesPage({ labels }: { labels: Labels }) {
  ...
  const defaultScores = groupStudents.map((student, index) => ({
    student,
    score: 78 + ((index * 7) % 20),
  }))
  const topStudent = [...defaultScores].sort((a, b) => b.score - a.score)[0]
  const average = Math.round(
    defaultScores.reduce((sum, item) => sum + item.score, 0) /
      Math.max(defaultScores.length, 1)
  )
  ...
  const exportGrades = () => {
    const rows = defaultScores.map(({ student, score }) => [
      student,
      scores[student] ?? String(score),
    ])
    const csv = [['Student', 'Score'], ...rows]
      .map((row) =>
  ...
  const saveGrade = (student: string) => {
    ...
    if (!Number.isFinite(score) || score < 0 || score > 100) {
      toast.error(labels.invalidScore)
      return
    }
  ...
  return (
    <>
      <PageHeading
        ...
        description={
          labels.locale === 'en'
            ? 'Select a group first, then review and update learner performance.'
            : 'Avval guruhni tanlang, keyin o‘quvchilar natijasini ko‘rib chiqing va baholang.'
        }
        labels={labels}
      />
      <div className='mb-4 grid gap-4 md:grid-cols-3'>
        <TeacherStat
          icon={Trophy}
          title={labels.locale === 'en' ? 'Top student' : 'Eng yaxshi o‘quvchi'}
          value={topStudent.student}
          detail={`${topStudent.score}%`}
        />
        <TeacherStat
          icon={Star}
          title={labels.avg}
          ...
          detail={
            labels.locale === 'en'
              ...
              : 'Joriy guruh o‘rtachasi'
          }
        />
        <TeacherStat
          icon={GraduationCap}
          title={labels.locale === 'en' ? 'Grade coverage' : 'Baholash qamrovi'}
          value={`${defaultScores.length}/${defaultScores.length}`}
          detail={
            labels.locale === 'en'
              ? 'Learners reviewed'
              : 'O‘quvchilar ko‘rib chiqildi'
          }
        />
      </div>
      ...
      <Card className='mb-4 overflow-hidden'>
        <CardHeader className='border-b bg-gradient-to-r from-primary/[0.08] via-card to-violet-500/[0.06]'>
          <CardTitle>
            {labels.locale === 'en' ? 'Choose a group' : 'Guruhni tanlang'}
          </CardTitle>
          <CardDescription>
            {labels.locale === 'en'
              ? 'The selected group controls the gradebook below.'
              : 'Tanlangan guruh pastdagi baholash jurnalini boshqaradi.'}
          </CardDescription>
        </CardHeader>
        <CardContent className='grid gap-3 p-4 md:grid-cols-3'>
          {groups.map((group) => (
            <button
              key={group.id}
              type='button'
              className={`cursor-pointer rounded-2xl border p-4 text-left transition-all ${selectedGroup === group.id ? 'border-primary bg-primary/[0.07] shadow-sm ring-1 ring-primary/20' : 'hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md'}`}
              onClick={() => setSelectedGroup(group.id)}
            >
              <div className='flex items-center justify-between'>
                <span className='font-semibold'>
      ...
      <div className='mb-4 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]'>
        <Card>
          <CardHeader>
            <CardTitle>{groupText(currentGroup, labels).name}</CardTitle>
            <CardDescription>
              {labels.locale === 'en'
                ? 'Score distribution for the selected group'
                : 'Tanlangan guruh baholarining taqsimoti'}
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-4'>
            {defaultScores.map(({ student, score }, index) => (
              <div key={student}>
                <div className='mb-2 flex items-center gap-3 text-sm'>
                  <span className='w-32 truncate font-medium'>{student}</span>
                  <div className='h-2 flex-1 rounded-full bg-muted'>
                    <div
                      className={`h-full rounded-full ${score >= 90 ? 'bg-emerald-500' : score >= 70 ? 'bg-primary' : 'bg-amber-500'}`}
                      style={{ width: `${score}%` }}
                    />
                  </div>
                ...
                {index === 0 && (
                  <p className='ms-32 text-[11px] text-muted-foreground'>
                    {labels.locale === 'en'
                      ? 'Top current result'
                      : 'Hozirgi eng yuqori natija'}
                  </p>
                )}
              </div>
            ))}
        ...
        <Card className='bg-gradient-to-br from-amber-500/[0.08] via-card to-primary/[0.05]'>
          <CardHeader>
            <CardTitle>
              {labels.locale === 'en' ? 'Group snapshot' : 'Guruh ko‘rinishi'}
            </CardTitle>
            <CardDescription>
              {labels.locale === 'en'
                ? 'Fast signals before entering grades.'
                : 'Baholashdan oldingi tezkor ko‘rsatkichlar.'}
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-3'>
            <div className='rounded-2xl border bg-background/70 p-4'>
              <p className='text-xs text-muted-foreground'>
                {labels.locale === 'en' ? 'Top learner' : 'Top o‘quvchi'}
              </p>
              <p className='mt-1 font-bold'>{topStudent.student}</p>
              <p className='mt-1 text-sm text-amber-600'>{topStudent.score}%</p>
            </div>
            <div className='grid grid-cols-2 gap-3'>
              <div className='rounded-2xl border bg-background/70 p-4'>
                <p className='text-xs text-muted-foreground'>
                  {labels.locale === 'en' ? '90+ scores' : '90+ baholar'}
                </p>
      ...
      <div className='mb-4 flex flex-wrap gap-2'>
        ...
        <Button
          ...
          className='cursor-pointer'
        >
          {labels.locale === 'en' ? 'Print / PDF' : 'Chop etish / PDF'}
        </Button>
      </div>
      <Card>
        <CardHeader className='border-b bg-gradient-to-r from-primary/[0.07] to-transparent'>
          <div className='flex items-center justify-between gap-3'>
            <div>
              <CardTitle>
                {labels.locale === 'en' ? 'Gradebook' : 'Baholash jurnali'}
              </CardTitle>
              <CardDescription>
                {labels.locale === 'en' ? 'Teacher:' : 'O‘qituvchi:'}{' '}
                {currentGroup.teacher} · {labels.avg}: {average}%
              </CardDescription>
            </div>
            <Badge>
              {currentGroup.students} {labels.students.toLowerCase()}
            </Badge>
          </div>
        </CardHeader>
        <CardContent className='space-y-2 pt-4'>
          {defaultScores.map(({ student, score }) => (
            <div
              key={student}
              className='grid gap-3 rounded-xl border p-3 transition-colors hover:bg-muted/20 sm:grid-cols-[1fr_120px_100px] sm:items-center'
            >
              <div className='flex items-center gap-3'>
                <span className='flex size-9 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary'>
                  {student
              ...
              <Input
                value={scores[student] ?? String(score)}
                onChange={(event) =>
                  setScores((current) => ({
                    ...current,
                ...
                type='text'
                inputMode='numeric'
                aria-label={`${student} ${labels.score}`}
              />
              <Button
                size='sm'
                variant='outline'
        ...
        <div className='relative flex flex-col gap-4 md:flex-row md:items-end md:justify-between'>
          ...
          <div className='rounded-2xl border bg-background/75 px-4 py-3 text-xs shadow-sm'>
            <p className='font-semibold'>
              {english ? 'Teacher workspace' : 'O‘qituvchi ish maydoni'}
            </p>
...
function TeacherStat({
  ...
  return (
    <Card className='teacher-stat'>
      <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
        <CardTitle className='text-sm font-medium'>{title}</CardTitle>
        <span className='teacher-stat-icon p-2'>
          <Icon className='size-4' />
        </span>
      </CardHeader>
      <CardContent>
