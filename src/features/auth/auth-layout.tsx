...
export function AuthLayout({ children }: AuthLayoutProps) {
  ...
  return (
    <div className='min-h-svh overflow-x-clip bg-[#f3fbf8] text-slate-950 lg:grid lg:h-svh lg:min-h-0 lg:grid-cols-2'>
      <section className='relative hidden min-h-0 overflow-hidden bg-[#001d16] text-white lg:flex lg:flex-col'>
        <div className='absolute inset-0 bg-[radial-gradient(circle_at_10%_20%,rgba(16,185,129,0.22),transparent_26%),radial-gradient(circle_at_82%_12%,rgba(20,184,166,0.16),transparent_22%),radial-gradient(circle_at_78%_88%,rgba(16,185,129,0.13),transparent_28%)]' />
        <div aria-hidden='true' className='absolute -start-16 top-[44%] size-64 -translate-y-1/2 -rotate-12 rounded-[2rem] border-[1.5px] border-[#00c98b]/20 bg-[#00c98b]/[0.035] shadow-[0_0_55px_rgba(0,201,139,0.08)]' />
        ...
        <div className='relative z-10 flex h-full flex-col p-10 xl:px-[4.25rem] xl:py-16'>
          <Link to='/sign-in' className='flex w-fit items-center gap-3 transition-opacity hover:opacity-80'>
            <div className='rounded-2xl bg-white p-2 shadow-[0_0_28px_rgba(16,185,129,0.35)]'>
          ...
          <div className='relative mt-auto max-w-2xl'>
            <div className='mb-6 inline-flex items-center gap-2 rounded-full border border-[#00c98b]/45 bg-[#00c98b]/10 px-3 py-1.5 text-xs font-semibold text-[#6df2c1]'>
            ...
            <h1 className='max-w-2xl text-5xl leading-[1.03] font-bold tracking-[-0.05em] text-[#f6fffc] xl:text-6xl'>
              {t('loginHeadlineBefore')} <span className='text-[#00d995]'>{t('loginHeadlineAccent')}</span> {t('loginHeadlineAfter')}
            </h1>
            <p className='mt-6 max-w-xl text-base leading-7 text-emerald-50/70'>{t('loginSecureAccess')}</p>
            <div className='mt-9 grid max-w-2xl grid-cols-3 gap-3'>
              {features.map((feature) => (
                <a key={feature.number} href={feature.href} className='group rounded-2xl border border-[#0a5b47]/75 bg-[#032e23]/95 p-4 shadow-[0_16px_42px_rgba(0,0,0,0.24)] transition-all hover:-translate-y-1 hover:border-[#00d995]/70 hover:bg-[#00c98b]/15 focus-visible:ring-2 focus-visible:ring-[#6df2c1]'>
                  <div className='flex items-start justify-between gap-2'><span className='flex size-9 items-center justify-center rounded-xl bg-[#00c98b]/20 text-lg text-[#6df2c1]'>{feature.icon}</span><span className='text-xs font-bold text-[#00d995]'>{feature.number}</span></div>
      ...
      <main className='relative flex min-h-svh min-w-0 items-center justify-center overflow-y-auto bg-[radial-gradient(circle_at_90%_8%,rgba(16,185,129,0.12),transparent_30%),linear-gradient(135deg,#f5fcfa_0%,#eef8f4_100%)] p-5 sm:p-10 lg:min-h-0 lg:overflow-hidden'>
        ...
        <div className='relative my-auto w-full max-w-[430px] lg:px-1'>
          ...
          <div className='relative'>
            <div aria-hidden='true' className='absolute -end-3 -bottom-3 -start-3 top-3 rounded-[1.25rem] border border-emerald-100/70 bg-white/40 shadow-[0_20px_48px_-30px_rgba(16,185,129,0.36)]' />
