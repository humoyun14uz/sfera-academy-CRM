...
export function SignUpForm({ className, ...props }: React.HTMLAttributes<HTMLFormElement>) {
  ...
  return <Form {...form}><form onSubmit={form.handleSubmit(onSubmit)} className={cn('grid gap-3.5', className)} {...props}>
    ...
    <Button className='mt-1 h-11 rounded-lg bg-[#29a956] text-[15px] font-bold shadow-[0_12px_24px_-10px_rgba(5,150,105,0.8)] transition-all hover:bg-[#218c48]' disabled={isLoading}>{isLoading ? <Loader2 className='animate-spin' /> : <UserPlus />}<span>{t('createAccount')}</span></Button>
  </form></Form>
}
