...
const groupNames = ['FR-02', 'FR-02', 'PY-01', 'FR-02', 'PY-01', 'FR-02']
type GroupChatUser = ChatUser & { group: string }

const staffConversations: GroupChatUser[] = conversations.map(
  (conversation, index) => ({
    ...conversation,
    title: staffRoles[index % staffRoles.length],
    fullName: staffNames[index % staffNames.length],
    group: groupNames[index % groupNames.length],
  })
)

function getRoleTranslationKey(role: string) {
  if (role === 'Super Admin') return 'superAdmin' as const
...
export function Chats() {
  ...
  const [chatList, setChatList] = useState<GroupChatUser[]>(staffConversations)
  const [selectedUser, setSelectedUser] = useState<GroupChatUser | null>(null)
  const [mobileSelectedUser, setMobileSelectedUser] =
    useState<GroupChatUser | null>(null)
  const [messageDraft, setMessageDraft] = useState('')
  const [createConversationDialogOpened, setCreateConversationDialog] =
    useState(false)

  // Filtered data based on the search query
  const filteredChatList = chatList.filter(({ fullName, group }) =>
    `${fullName} ${group}`.toLowerCase().includes(search.trim().toLowerCase())
  )
  ...
  const sendMessage = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const nextUser = {
      ...selectedUser,
      messages: [nextMessage, ...selectedUser.messages],
    }
    setChatList((items) =>
      items.map((item) => (item.id === nextUser.id ? nextUser : item))
    )
                      ...
                      <div className='flex gap-2'>
                        ...
                        <div>
                          ...
                          <span className='block text-xs text-primary'>
                            {t(getRoleTranslationKey(chatUsr.title))}
                          </span>
                ...
                <div className='flex gap-3'>
                  ...
                  <div className='flex items-center gap-2 lg:gap-4'>
                    ...
                    <div>
                      ...
                      <span className='col-start-2 row-span-2 row-start-2 flex items-center gap-2 text-xs text-muted-foreground lg:text-sm'>
                        {t(getRoleTranslationKey(selectedUser.title))}
                ...
                >
                  <div className='flex flex-1 items-center gap-2 rounded-md border border-input bg-card px-2 py-1 focus-within:ring-1 focus-within:ring-ring focus-within:outline-hidden lg:gap-4'>
                    ...
                    <label className='flex-1'>
                      <span className='sr-only'>{t('chatTextBox')}</span>
                      <input
                        type='text'
