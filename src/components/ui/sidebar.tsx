    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            'group/sidebar-wrapper relative flex min-h-svh w-full min-w-0 has-data-[variant=inset]:bg-sidebar',
            className
          )}
          {...props}
        >
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot='sidebar'
          data-mobile='true'
          className='w-(--sidebar-width) border-e border-sidebar-border/70 bg-sidebar p-0 text-sidebar-foreground shadow-xl [&>button]:hidden'
          style={
            {
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          variant === 'floating' || variant === 'inset'
            ? 'relative px-2 pt-0 pb-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4))+2px)] group-data-[side=left]:border-e group-data-[side=left]:border-border group-data-[side=right]:border-s group-data-[side=right]:border-border'
      ...
      >
        <div
          ...
          className='flex h-full w-full flex-col bg-sidebar group-data-[variant=floating]:rounded-2xl group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow-[0_8px_30px_rgb(15_23_42/0.08)]'
        >
          {children}
        </div>
      </div>
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        'absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-inset-e-2 group-data-[side=right]:inset-s-0 sm:flex',
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'md:peer-data-[variant=inset]:m-0 md:peer-data-[variant=inset]:rounded-none md:peer-data-[variant=inset]:shadow-none md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ms-0',
        className
      )}
      {...props}
    />
...
function SidebarHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar='header'
      className={cn(
        'flex h-16 shrink-0 flex-col justify-center gap-2 border-b-0 p-2',
        className
      )}
      {...props}
    />
...
function SidebarContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex min-h-0 flex-1 flex-col gap-2 overflow-x-hidden overflow-y-auto overscroll-contain [scrollbar-width:none] [&::-webkit-scrollbar]:hidden',
        className
      )}
      {...props}
    />
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
  return (
    <ul
      ...
      className={cn(
        'mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-s border-border px-2.5 py-0.5',
        'group-data-[collapsible=icon]:hidden',
        className
      )}
      {...props}
